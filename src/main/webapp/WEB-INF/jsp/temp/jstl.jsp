...
<%-- 				<c:import url="/temp/selectlist.do" charEncoding="utf-8"/> --%>
		...
		<c:when test="${step eq '3'}">
			<li>
				...
				<c:set var="str" value="지금은 c:set연습중"/>
				<c:out value="${str}"/>
			</li>	
		</c:when>
		...
		<c:when test ="${step eq '5'}">
			...
			<c:when test="${step eq '7'}">
				<li>
					<h3>c:param Tag : 포함하는 import, url 태그의 URL의 매개 변수를 추가 (어딘가에 파라미터를 보내야할 때 param을 쓴다.)
					jstlimport.jsp에 value값 작성.</h3>
